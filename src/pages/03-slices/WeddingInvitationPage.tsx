...
export const WeddingInvitationPage = () => {
  ...
  const eventFecha = useWeddingBoundStore(state => state.eventFecha());
  const eventTiempo = useWeddingBoundStore(state => state.eventTiempo());

  const setEventDate = useWeddingBoundStore(state => state.setEvenDate);
  const setEvenTime = useWeddingBoundStore(state => state.setEvenTime);

  const confirmation = useWeddingBoundStore(state => state.confirm);
  const setConfirm = useWeddingBoundStore(state => state.setConfirm);

  const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    console.log({firstName,lastName, guestCount, eventFecha, eventTiempo, confirmation})
  }
